...
// Force Dedicated Graphics Card (NVIDIA / AMD)
extern "C" {
...
char G_cCmdLine[256], G_cCmdLineTokenA[120], G_cCmdLineTokenA_Lowercase[120],
    G_cCmdLineTokenB[120], G_cCmdLineTokenC[120], G_cCmdLineTokenD[120],
    G_cCmdLineTokenE[120];
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  case WM_CLOSE:
    if ((G_pGame->m_cGameMode == DEF_GAMEMODE_ONMAINGAME) &&
        (G_pGame->m_bForceDisconn == false)) {

#ifdef _DEBUG
      if (G_pGame->m_cLogOutCount == -1 || G_pGame->m_cLogOutCount > 2)
        G_pGame->m_cLogOutCount = 1;
#else
      if (G_pGame->m_cLogOutCount == -1 || G_pGame->m_cLogOutCount > 11)
  ...
  case WM_ACTIVATEAPP:
    if (wParam == 0) {
      ...
      if (G_pGame->bCheckImportantFile() == false) {
        MessageBox(G_pGame->m_hWnd,
                   "File checksum error! Get Update again please!", "ERROR1",
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  timeBeginPeriod(1); // Enable High Resolution Timer (1ms)
  HINSTANCE hDll;
  ...
  if (OpenMutex(MUTEX_ALL_ACCESS, false, "0543kjg3j31%") != 0) {
    MessageBox(0, "Only one Helbreath client program allowed!", "ERROR!",
               MB_OK);
    return 0;
  }
...
bool InitInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  // Borderless Fullscreen Window (User Request)
  int screenW = GetSystemMetrics(SM_CXSCREEN);
  int screenH = GetSystemMetrics(SM_CYSCREEN);

  G_hWnd =
      CreateWindowEx(0, szAppClass, "Helbreath Client", WS_POPUP | WS_VISIBLE,
  ...
  if (cy > 340)
    cy -= 40;
  G_hWnd = CreateWindowEx(0, szAppClass, "Helbreath", WS_POPUP, cx - 400,
                          cy - 300, 800, 600, 0, 0, hInstance, 0);
#endif
  if (!G_hWnd)
    return false;
...
void EventLoop() {
  ...
  while (true) {
    if (PeekMessage(&msg, 0, 0, 0, PM_NOREMOVE)) {
      ...
      if ((dwCurrentTime - dwLastFrame) >= 7) {
        dwLastFrame = dwCurrentTime;
        G_pGame->UpdateScreen();
      } else {
        if ((7 - (dwCurrentTime - dwLastFrame)) > 2)
          Sleep(1);
        else
          Sleep(0);
      }
...
void Initialize(char *pCmdLine) {
  ...
  // CPU Optimization for i9 12900K / Modern CPUs
  // Force High Priority and Lock to first 2 cores (Likely P-Cores 0 and 1)
  SetPriorityClass(GetCurrentProcess(), HIGH_PRIORITY_CLASS);
  SetProcessAffinityMask(GetCurrentProcess(), 0x00000003); // Cores 0 and 1

  if (iErrCode) {
    MessageBox(G_hWnd, "Winsock-V1.1 not found! Cannot execute program.",
