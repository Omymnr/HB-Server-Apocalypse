...
bool DXC_ddraw::bInit(HWND hWnd) {
  ...
  // Force Windowed Mode logic explicitly (Safety Net)
#ifdef DEF_WINDOWED_MODE
  m_bFullMode = false;
#endif
  ...
  if (m_bFullMode == true) {
    ...
    ddVal = m_lpDD4->SetDisplayMode(res_x, res_y, 16, 144, 0); // Try 144Hz
    if (ddVal != DD_OK)
      ddVal = m_lpDD4->SetDisplayMode(res_x, res_y, 16, 120, 0); // Try 120Hz
    if (ddVal != DD_OK)
      ddVal = m_lpDD4->SetDisplayMode(res_x, res_y, 16, 0, 0); // Default
    return false;
    ...
    if (ddVal != DD_OK)
      return false;
    SetRect(&m_rcFlipping, 0, 0, res_x,
            res_y); // our fictitious sprite bitmap is
  } else {
    ...
    if (cy > 340)
      cy -= 40;
    // Enforce Borderless Fullscreen Style and Size
    ...
    // Force Position and Size (0,0 -> Screen Width, Screen Height)
    ...
    SetRect(&m_rcFlipping, cx - res_x_mid, cy - res_y_mid, cx + res_x_mid,
            cy + res_y_mid); // our fictitious sprite bitmap is
  }
  ...
  ddsd.dwSize = sizeof(ddsd);
  if (m_lpBackB4->Lock(0, &ddsd, DDLOCK_WAIT, 0) != DD_OK)
    return false;
  ...
  m_hFontInUse = 0;
  m_hFontInUse =
      CreateFont(16, 0, 0, 0, FW_NORMAL, false, false, false, ANSI_CHARSET,
                 OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                 NONANTIALIASED_QUALITY, VARIABLE_PITCH, "Tahoma");
  m_hDC = 0;
  m_init = true;
  return true;
}
...
HRESULT DXC_ddraw::iFlip() {
  ...
  if (m_bFullMode) {
    ...
    ddVal = m_lpFrontB4->Blt(0, m_lpBackB4, 0, DDBLT_WAIT, 0);
    if (G_hEditWnd != 0) {
    ...
    ddVal = m_lpBackB4flip->BltFast(0, 0, m_lpBackB4, &m_rcFlipping,
                                    DDBLTFAST_NOCOLORKEY); // | DDBLTFAST_WAIT);
    ddVal = m_lpFrontB4->Flip(m_lpBackB4flip, DDFLIP_WAIT);

#endif
  } else {
    // Dynamic scaling for Borderless Window
    // Get the actual window client area (which is now full screen)
    RECT rcDest;
    GetClientRect(G_hWnd, &rcDest);

    // Convert client coordinates to screen coordinates if using a specific
    // clipper, BUT since we are using pClipper->SetHWnd(0, hWnd), the clipper
    // handles the offset usually. However, Blt to primary surface usually
    // expects screen coordinates.
    POINT pt = {0, 0};
    ClientToScreen(G_hWnd, &pt);
    OffsetRect(&rcDest, pt.x, pt.y);

    ddVal = m_lpFrontB4->Blt(&rcDest, m_lpBackB4, 0, DDBLT_WAIT, 0);
  }
  ...
  if (ddVal == DDERR_SURFACELOST) {
    ...
    if (m_lpBackB4->Lock(0, &ddsd2, DDLOCK_WAIT, 0) != DD_OK)
      return false;
...
void DXC_ddraw::ChangeDisplayMode(HWND hWnd) {
  ...
  if (m_bFullMode == false) {
    ...
    if (cy > 340)
      cy -= 40;
    // SetWindowPos(hWnd, HWND_TOP, cx - res_x_mid, cy - res_y_mid, res_x,
    // res_y,
    //             SWP_SHOWWINDOW);
...
IDirectDrawSurface7 *DXC_ddraw::pCreateOffScreenSurface(WORD wSzX, WORD wSzY) {
  ...
  if (m_lpDD4->CreateSurface(&ddsd, &pdds4, 0) != DD_OK)
    return 0;
  return pdds4;
}
...
void DXC_ddraw::TextOut(int x, int y, char *cStr, COLORREF rgb) {
  bool bManualLock = (m_iDCLockCount > 0);
  if (!bManualLock)
    _GetBackBufferDC();
...
long DXC_ddraw::_CalcMaxValue(int iS, int iD, char cMode, char cMethod,
                              double dAlpha) {
  long Sum;
  double dTmp;
...
void DXC_ddraw::DrawShadowBox(short sX, short sY, short dX, short dY,
                              int iType) {
  WORD *pDst, wValue;
  int ix, iy;
...
void DXC_ddraw::_ReleaseBackBufferDC() { m_lpBackB4->ReleaseDC(m_hDC); }

void DXC_ddraw::BeginGDI() {
  if (m_iDCLockCount == 0) {
    _GetBackBufferDC();
  }
  m_iDCLockCount++;
...
void DXC_ddraw::DrawText(LPRECT pRect, const char *pString, COLORREF rgb) {
  bool bManualLock = (m_iDCLockCount > 0);
  if (!bManualLock)
    _GetBackBufferDC(); // Manual lock not active? Auto-lock (Legacy behavior)

  SetTextColor(m_hDC, rgb);
  ::DrawText(m_hDC, pString, strlen(pString), pRect,
             DT_CENTER | DT_NOCLIP | DT_WORDBREAK | DT_NOPREFIX); // v2.15

  if (!bManualLock)
    _ReleaseBackBufferDC(); // Auto-unlock
}
...
bool DXC_ddraw::Screenshot(LPCTSTR FileName, LPDIRECTDRAWSURFACE7 lpDDS) {
  ...
  try {
    ...
    if ((OffscrDC = CreateCompatibleDC(SurfDC)) == 0)
      throw 3;
    ...
    if ((BmpFile = CreateFile(FileName, GENERIC_WRITE, 0, 0, CREATE_ALWAYS,
    ...
    if (!WriteFile(BmpFile, lpvBits, lpbi->bmiHeader.biSizeImage, &Written, 0))
      throw 12;
...
void DXC_ddraw::DrawItemShadowBox(short sX, short sY, short dX, short dY,
                                  int iType) {
  WORD *pDst, wValue;
  int ix, iy;
