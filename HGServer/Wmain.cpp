      ...
      if (G_pGame->m_pMapList[a] != 0) {
        for (int iy = 0; iy < G_pGame->m_pMapList[a]->m_sSizeY; iy++) {
          for (int ix = 0; ix < G_pGame->m_pMapList[a]->m_sSizeX; ix++) {
            pTile = (class CTile *)(G_pGame->m_pMapList[a]->m_pTile + ix +
                                    iy * G_pGame->m_pMapList[a]->m_sSizeY);
            if ((pTile != 0) && (pTile->m_sOwner != 0) &&
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  default:
    /*if ((message >= WM_ONLOGSOCKETEVENT + 1) && (message <=
       WM_ONLOGSOCKETEVENT + DEF_MAXSUBLOGSOCK))
            G_pGame->OnSubLogSocketEvent(message, wParam, lParam);*/

    if ((message >= WM_USER_BOT_ACCEPT + 1) &&
        (message <= WM_USER_BOT_ACCEPT + DEF_MAXCLIENTLOGINSOCK))
      G_pGame->OnSubLogSocketEvent(message, wParam, lParam);
...
/*void GetOSName(){
        ...
        case VER_PLATFORM_WIN32_NT:
                ...
                if ( osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 2 )
                        strcat(G_cCrashTxt,"Microsoft Windows Server 2003
family, ");
...
); else if( osvi.wSuiteMask & VER_SUITE_PERSONAL ) strcat(G_cCrashTxt, "Home
Edition " ); else strcat(G_cCrashTxt, "Professional " );
                        }
...
osvi.dwMinorVersion == 2 )
                                {
                                        if( osvi.wSuiteMask &
VER_SUITE_DATACENTER ) strcat(G_cCrashTxt, "Datacenter Edition " ); else if(
osvi.wSuiteMask & VER_SUITE_ENTERPRISE ) strcat(G_cCrashTxt, "Enterprise Edition
" ); else if ( osvi.wSuiteMask == VER_SUITE_BLADE ) strcat(G_cCrashTxt, "Web
Edition " ); else strcat(G_cCrashTxt, "Standard Edition " );
                                }
...
osvi.dwMinorVersion == 0 )
                                {
                                        if( osvi.wSuiteMask &
VER_SUITE_DATACENTER ) strcat(G_cCrashTxt, "Datacenter Server " ); else if(
osvi.wSuiteMask & VER_SUITE_ENTERPRISE ) strcat(G_cCrashTxt, "Advanced Server "
); else strcat(G_cCrashTxt, "Server " );
                                }

                                else  // Windows NT 4.0
                                {
                                        if( osvi.wSuiteMask &
VER_SUITE_ENTERPRISE ) strcat(G_cCrashTxt,"Server 4.0, Enterprise Edition " );
                                        else
...
earlier
                {
                        ...
                        wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "%d.%d ",
...
NT\\CurrentVersion\\Hotfix\\Q246009", 0, KEY_QUERY_VALUE, &hKey ); if( lRet ==
ERROR_SUCCESS ) wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "Service Pack 6a
(Build %d)\r\n", osvi.dwBuildNumber & 0xFFFF ); else // Windows NT 4.0 prior to
SP6a
                        {
                                wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "%s
(Build %d)\r\n", osvi.szCSDVersion, osvi.dwBuildNumber & 0xFFFF);
                ...
                {
                        wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "%s (Build
%d)\r\n", osvi.szCSDVersion, osvi.dwBuildNumber & 0xFFFF);
                }
...
== 'B' ) strcat(G_cCrashTxt,"OSR2 " );
                ...
                {
                        strcat(G_cCrashTxt,"Microsoft Windows Millennium
Edition\r\n");
                }
                break;
...
/*bool CALLBACK lpCrashDialogFunc(HWND hDlg,UINT uMsg,WPARAM wParam,LPARAM
lParam){ HANDLE outHand; char cCrashFileName[MAX_PATH]; char cLF[]={0x0d,0x0a};
char cDash ='-';
SYSTEMTIME sysTime;
DWORD written;
        ...
        case WM_INITDIALOG:
                ...
                wsprintf(cCrashFileName,"CrashData - %d-%d-%d.txt",
sysTime.wDay, sysTime.wMonth, sysTime.wYear); SetWindowText(GetDlgItem(hDlg,
IDC_EDITPATH), cCrashFileName);
                //Open File For Writing
                outHand =
CreateFile(cCrashFileName,GENERIC_READ+GENERIC_WRITE,FILE_SHARE_READ+FILE_SHARE_WRITE,0,OPEN_ALWAYS,FILE_ATTRIBUTE_NORMAL,0);
                SetFilePointer(outHand, 0, 0, FILE_END);
                WriteFile(outHand, G_cCrashTxt, strlen(G_cCrashTxt), &written,
0); for (int i = 0; i < 80; i++) WriteFile(outHand, &cDash, 1, &written, 0);
...
ExceptionInfo->ExceptionRecord->ExceptionCode); strcat(G_cCrashTxt, "Flags : ");
        wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "0x%.8X\r\n",
ExceptionInfo->ExceptionRecord->ExceptionFlags); strcat(G_cCrashTxt, "Address :
"); wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "0x%.8X\r\n",
ExceptionInfo->ExceptionRecord->ExceptionAddress); strcat(G_cCrashTxt,
"Parameters : "); wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "0x%.8X\r\n\r\n",
ExceptionInfo->ExceptionRecord->NumberParameters);
        ...
        wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "EDI: 0x%.8X\t\tESI:
0x%.8X\t\tEAX: 0x%.8X\r\n",ExceptionInfo->ContextRecord->Edi,
                                                                                                                                                                                ExceptionInfo->ContextRecord->Esi,
                                                                                                                                                                                ExceptionInfo->ContextRecord->Eax);
        wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "EBX: 0x%.8X\t\tECX:
0x%.8X\t\tEDX: 0x%.8X\r\n",ExceptionInfo->ContextRecord->Ebx,
                                                                                                                                                                                ExceptionInfo->ContextRecord->Ecx,
                                                                                                                                                                                ExceptionInfo->ContextRecord->Edx);
        wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "EIP: 0x%.8X\t\tEBP:
0x%.8X\t\tSegCs: 0x%.8X\r\n",ExceptionInfo->ContextRecord->Eip,
                                                                                                                                                                                ExceptionInfo->ContextRecord->Ebp,
                                                                                                                                                                                ExceptionInfo->ContextRecord->SegCs);
        wsprintf(&G_cCrashTxt[strlen(G_cCrashTxt)], "EFlags: 0x%.8X\tESP:
0x%.8X\t\tSegSs: 0x%.8X\r\n",ExceptionInfo->ContextRecord->EFlags,
                                                                                                                                                                                ExceptionInfo->ContextRecord->Esp,
                                                                                                                                                                                ExceptionInfo->ContextRecord->SegSs);
        // Show Dialog
        DialogBox(GetModuleHandle(0), MAKEINTRESOURCE(IDD_DIALOG2), 0,
(DLGPROC)lpCrashDialogFunc); SendMessage(0, WM_CLOSE, 0, 0); return
...
bool InitInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  // hFile = hFile = CreateFile("HGserver.exe", GENERIC_READ, 0, 0,
  // OPEN_EXISTING, 0, 0); if (hFile == INVALID_HANDLE_VALUE) return false;
  // GetFileTime(hFile, &ftCT, &ftLAT, &ftLWT);
  // CloseHandle(hFile);

  GetLocalTime(&SysTime);
  // wsprintf(cTitle, "Helbreath Korean Test Server Version %s.%s %d (Executed
  // at: %d %d %d)", DEF_UPPERVERSION, DEF_LOWERVERSION, DEF_BUILDDATE,
  // SysTime.wMonth, SysTime.wDay, SysTime.wHour);
  wsprintf(cTitle, "Helbreath GameServer V%s.%s %d (Executed at: %d %d %d)",
           DEF_UPPERVERSION, DEF_LOWERVERSION, DEF_BUILDDATE, SysTime.wMonth,
...
void Initialize() {
  ...
  if (_InitWinsock() == false) {
    MessageBox(G_hWnd, "Socket 1.1 not found! Cannot execute program.", "ERROR",
...
void OnAcceptLogin() { G_pGame->bAcceptLogin(G_pLoginSock); }
...
void OnAccept() { G_pGame->bAccept(G_pListenSock); }

void CALLBACK _TimerFunc(UINT wID, UINT wUser, DWORD dwUSer, DWORD dw1,
                         DWORD dw2) {
...
void PutLogFileList(char *cStr) {
  ...
  GetLocalTime(&SysTime);
  wsprintf(cBuffer, "(%4d:%2d:%2d:%2d:%2d) - ", SysTime.wYear, SysTime.wMonth,
